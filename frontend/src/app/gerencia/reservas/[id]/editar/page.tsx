...
export default function EditarReservaGerenciaPage() {
  ...
  const loadData = async () => {
    try {
      const [reservaResponse, tiposResponse] = await Promise.all([
        api.get(`/gerencia/reservas/${params.id}`),
        api.get('/gerencia/tipos-quarto'),
      ]);
      ...
      setFormData({
        ...
        hospedes: reservaData.hospedes?.map((h: any) => ({
          nome: h.hospede.nome,
          tipoDocumento: h.hospede.tipoDocumento,
          numeroDocumento: h.hospede.numeroDocumento,
          nif: h.hospede.nif || null,
        })) || [],
      });
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/gerencia/reservas/${params.id}`, formData);
      router.push('/gerencia/reservas?success=editada');
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4">
          <Link href="/gerencia/reservas" className="text-primary-600 hover:text-primary-700">
            ← Voltar
          </Link>
        </div>
      </nav>
            ...
            <div>
              ...
              {tipoQuartoSelecionado && (
                <p className="text-xs text-gray-500 mt-1">
                  Capacidade base: {tipoQuartoSelecionado.capacidadeBase} hóspedes (máximo: {tipoQuartoSelecionado.capacidadeBase + 2})
                </p>
              )}
            </div>
          </div>
